...
export const Schedule = () => {
  ...
  const [isTeamScheduleSelected, setIsTeamScheduleSelected] = React.useState(
    true,
  )
  ...
  const getTeamScheduleInfo = async () => {
    try {
      ...
      const allPlayers = await YogiCupService.getAllPlayers()
      const currentPlayer = allPlayers.find(
        (player) => player.userId === userId,
      )
      const teamId = currentPlayer ? currentPlayer.teamId : -1
